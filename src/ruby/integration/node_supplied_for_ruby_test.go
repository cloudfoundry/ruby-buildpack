...
var _ = Describe("running supply nodejs buildpack before the ruby buildpack", func() {
	...
	Context("the app is pushed", func() {
		...
		It("finds the supplied dependency in the runtime container", func() {
			...
			Expect(app.Stdout.String()).To(ContainSubstring("Installing node 18."))

			body, err := app.GetBody("/")
			Expect(err).To(BeNil())
			Expect(body).To(ContainSubstring("Ruby version: ruby 3."))
			Expect(body).To(ContainSubstring("Node version: v18."))
